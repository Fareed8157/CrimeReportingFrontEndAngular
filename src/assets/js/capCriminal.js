var numberOfItems = $('#cardViewForCriminal .row .col-md-3').length;

var limitPerPage = 4; // Limit of items per each page
$('#cardViewForCriminal .row .col-md-3:gt(' + (limitPerPage - 1) + ')').hide(); // Hide all items over page limits (e.g., 5th item, 6th item, etc.)
...
$(".pagination li.current-page").on("click", function() {
  ...
  if ($(this).hasClass('active')) {
    ...
    $("#cardViewForCriminal .row .col-md-3").hide(); // Hide all items in loop, this case, all the list groups
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page number that was clicked on

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#cardViewForCriminal .row .col-md-3:eq(" + i + ")").show(); // Show items from the new page that was selected
    }
  }
...
$("#next-page").on("click", function() {
  ...
  if (currentPage === totalPages) {
    ...
    $("#cardViewForCriminal .row .col-md-3").hide(); // Hide all items in the pagination loop
